...
**Multi-tier model routing.** Route low-consequence agent steps (metadata extraction, intent classification, query formatting) to a fast lightweight model and reserve frontier model calls for high-consequence steps (compliance synthesis, sufficiency assessment, draft generation). Requires a routing classifier, per-step consequence classification in the trust ledger schema, and separate cost baselines per model tier. Reference: DL-040.
