...
Prompt caching implemented for sufficiency and drafting system prompts (see `src/agent/llm.py` — `_invoke_cached`, `_get_caching_client`). Confirmed finding: this account has access only to the cross-region inference profile (`us.anthropic.claude-sonnet-4-5-20250929-v1:0`); the base model ID (`anthropic.claude-sonnet-4-5-20251001-v1:0`) returns `ValidationException: The provided model identifier is invalid`. Bedrock prompt caching via the `anthropic-beta` header requires the base model ID — it is not available via cross-region inference profiles. The caching infrastructure is complete and correct; cache metrics will populate automatically if the account is granted access to a base model ID. Token baseline uses the uncached cross-region profile numbers from the table above.
